...
\begin{framed}
	\lstinputlisting[language=bash]{files/project.properties-controleacesso}
\end{framed}
...
\begin{framed}
	\lstinputlisting[language=xml]{files/login-config-controleacesso.xml}
\end{framed}
...
Em seguida, executaremos o seguinte script para iniciliazar o banco com o
usuário \texttt{su} com a senha \texttt{'su'} para o \texttt{sistemaacademico}:

\begin{framed}
	\lstinputlisting[language=sql]{files/init-bd-controleacesso.sql}
\end{framed}
...
\item Rode, agora, o seguinte script no banco de dados do \texttt{Controle de
Acesso}:
\begin{framed}
	\lstinputlisting[language=sql]{files/acao_pf.sql}
\end{framed}
