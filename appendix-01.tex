...
\item Criar o descritor de configuração do cartucho
(\texttt{andromda-cartridge.xml}).
\item Criar os \texttt{Templates Velocity}.
\item Compilar o \texttt{MDArte} junto com .
\item Instalar o cartucho desenvolvido em um repositório para o \texttt{Maven}.
\end{itemize}
...
Antes de qualquer etapa, precisamos garantir que a estrutura de diretório em
disco, onde iremos armazenar todos os artefatos para a criação de um cartucho,
esteja correta. Para isso precisamos seguir a convenção abaixo como na imagem:

\begin{figure}[H]
	\centering
	\includegraphics[width=180pt,height=155pt]{files/imgs/apendice-cartucho-novo-0000.png}
	\caption{Estrutura de diretórios de um cartucho MDArte}
	\label{field_type_autocomplete}
\end{figure}

\texttt{java} - Diretório com as classes \texttt{Java} para as Metafacades.
\texttt{META-INF} - Diretório com os arquivos \texttt{XML} de configuração.
\texttt{templates} - Diretório com os \texttt{Templates Velocity} (Arquivos
\texttt{VSL}).
\texttt{uml} - Diretório com o metamodelo de dados armazenado em um arquivo
\texttt{XMI}.

Em alguns casos, será necessário a criação do subdiretório resources, dentro do
diretório \texttt{src}. Este irá conter todos os recursos que serão apenas
copiados pelo cartucho (Veja logo a seguir o elemento \texttt{<resource/>} para
maiores explicações).

\section{Criando o Descritor de Configuração do Cartucho}
Uma vez criada a estrutura de diretórios, o próximo passo será a criação do
descritor de configuração do cartucho. O descritor de configuração nada mais é
que um arquivo \texttt{XML}, com o nome \texttt{andromda-cartridge.xml}, e
...
\begin{enumerate}
\item <cartridge/> - Elemento raiz para o descritor de configuração do cartucho.
...
utilizados, caso necessário, pelos Templates Velocity. Possui os
seguintes atributos:
...
processado.
Obs: Uma variável do Template Velocity pode ser informada (Ex.: \textdollar{}
generatedName) desde que esta seja configurada no Template correspondente.
...
<modelElements/>
	Este elemento aninhado com o elemento <template/>, é utilizado para informar ao Template Velocity quais os elementos de modelo deverão ser processados.
	Atributos:
variable - Define a variável (Ex.: \textdollar{}entity) que estará disponibilizada
para utilização no Template Velocity.

<modelElement/>
	Este elemento aninhado com o elemento <modelElements/>, é utilizado para informar ao Template Velocity qual elemento de modelo deverá ser processado.
	Atributos:
stereotype - Define o nome do esteriótipo que o elemento de modelo precisa ter para que este esteja disponível para uso no Template Velocity. Este campo não é obrigatório, pois o elemento aninhado <type/> pode ser utilizado, ao invés deste atributo.
variable - Define a variável que estará disponibilizada para utilização no Template Velocity. O uso deste atributo apenas terá sentido caso o atributo outputToSingleFile esteja com o valor true, atentando ao fato de que sendo suficiente, na maioria dos casos, apenas o atributo variable para o elemento <modelElements/>.
Em combinação com o atributo outputToSingleFile, este atributo permite agrupar elementos de modelo e torná-los disponíveis para os Templates Velocity.



No exemplo acima, todos os elementos de modelo com os esteriótipos
