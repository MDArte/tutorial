...
\begin{figure}[H]
	\centering
	\includegraphics[width=180pt,height=155pt]{files/imgs/apendice-cartucho-novo-00000.png}
	\caption{Estrutura de diretórios de um cartucho MDArte}
	\label{field_type_autocomplete}
\end{figure}

\begin{itemize}
	\item \texttt{java} - Diretório com as classes \texttt{Java} para as
Metafacades.
	\item \texttt{META-INF} - Diretório com os arquivos \texttt{XML} de
	configuração.
	\item \texttt{templates} - Diretório com os \texttt{Templates Velocity}
	(Arquivos \texttt{VSL}).
	\item \texttt{uml} - Diretório com o metamodelo de dados armazenado em um
	arquivo \texttt{XMI}.
\end{itemize}
...
no Template Velocity. O uso deste atributo apenas terá sentido caso o atributo outputToSingleFile esteja com o valor true, atentando ao fato de que sendo suficiente, na maioria dos casos, apenas o atributo variable para o elemento <modelElements/>.
